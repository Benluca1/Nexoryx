...
def run_bot(stop_event=None) -> int:
    """Haupt-Loop. stop_event: threading.Event zum sauberen Beenden aus einem Thread."""
    ...
    try:
        while not (stop_event and stop_event.is_set()):
            ...
            for upd in updates.get("result", []):
                ...
                if role is None:
                    _send(token, chat_id,
                          f"Zugriff verweigert. Deine ID: {user_id}\n"
...
def start_background():
    """Startet den Bot als Daemon-Thread wenn TELEGRAM_BOT_TOKEN gesetzt ist.

    Wird von nexoryxd automatisch beim Start aufgerufen — kein manueller Befehl nötig.
    Gibt den Thread zurück oder None wenn kein Token konfiguriert ist.
    """
    import threading
    token = cfg_mod.get_key("TELEGRAM_BOT_TOKEN")
    if not token:
        return None
    t = threading.Thread(target=run_bot, daemon=True, name="nexoryx-telegram")
    t.start()
    return t
